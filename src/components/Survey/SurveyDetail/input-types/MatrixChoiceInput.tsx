...
const MatrixChoiceInput: FC<Props> = ({
  matrixRows,
  matrixColumns,
  onChange,
  isReadOnly,
  ...
  const handleAddColumn = () => {
    if (onChange) {
      const newColumn: MatrixColumn = {
        label: `Option ${matrixColumns.length + 1}`,
  ...
  return (
    <VStack align="start" spacing={6} w="full">
      <Text fontSize="sm" color="gray.500">
        * Matrix choice: User will select one option for each row
      </Text>
      ...
      {/* Table 2: Answer Options (Columns) */}
      <Box w="full">
                ...
                <Tr key={colIndex}>
                  ...
                  <Td>
                    <Input
                      ...
                      size="sm"
                      placeholder={`Enter option ${colIndex + 1}`}
                      isReadOnly={isReadOnly}
                      isDisabled={isDisabled}
                    />
                  </Td>
...
export default MatrixChoiceInput;
